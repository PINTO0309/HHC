#!/usr/bin/env python3
"""Generate a class-ratio pie chart PNG from the HHC parquet dataset."""
...
LABEL_ORDER = ["no_wearing_hat", "wearing_hat"]
LABEL_DISPLAY = {
    "no_wearing_hat": "No Wearing Hat",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a pie chart PNG for class composition in the HHC parquet dataset."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--title",
        default=None,
        help="Optional chart title. Defaults to an HHC dataset title.",
    )
...
def main() -> None:
    ...
    title = args.title or f"HHC dataset class ratio ({args.input.name})"
    ax.set_title(
        f"{title}\nTotal: {total:,} samples",
        fontsize=14,
        pad=18,
