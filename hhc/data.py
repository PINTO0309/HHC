...
class Sample:
    """Lightweight container describing a single hat-classification image."""
...
POSITIVE_LABELS = {"wearing_hat", "with_hat"}
NEGATIVE_LABELS = {"no_wearing_hat", "without_hat"}
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "wearing_hat" if label == 1 else "no_wearing_hat"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'wearing_hat').",
            split,
            total,
...
class HHCDataset(Dataset):
    """Torch dataset that reads hat-classification images from disk or embedded bytes."""
