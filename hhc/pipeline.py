...
from .model import HHC, ModelConfig

LOGGER = logging.getLogger("hhc")

LABEL_MAP = {0: "no_wearing_hat", 1: "wearing_hat"}
...
def _parse_image_size_arg(raw: Any) -> tuple[int, int]:
    ...
    raise argparse.ArgumentTypeError(
        "Image size must be specified as a single integer (e.g. '32') or as 'HEIGHTxWIDTH' (e.g. '32x32')."
    )
                ...
                results.append(
                    {
                        ...
                        "prob_wearing_hat": float(probs[idx].detach().cpu().item()),
                    }
                )
    return results
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    train_dataset = HHCDataset(train_samples, transform=train_transform)
    val_dataset = HHCDataset(splits["val"], transform=eval_transform) if splits["val"] else None
    test_dataset = HHCDataset(splits["test"], transform=eval_transform) if splits["test"] else None
    ...
    model = HHC(model_config).to(device)
    base_metadata = {
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"hhc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "hhc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"hhc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "hhc_best_", 10)
...
def predict_images(
    ...
    image_size_raw = normalization.get("image_size", (32, 32))
    try:
        image_size = _ensure_image_size_tuple(image_size_raw)
    except ValueError:
        LOGGER.warning("Invalid image_size %s in checkpoint; defaulting to 32x32.", image_size_raw)
        image_size = (32, 32)
...
def export_to_onnx(
    ...
    torch.onnx.export(
        ...
        output_path,
        input_names=["images"],
        output_names=["prob_wearing_hat"],
        dynamic_axes={"images": {0: "batch"}, "prob_wearing_hat": {0: "batch"}},
        opset_version=opset,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="HHC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the HHC binary classifier from parquet data.")
    ...
    train_parser.add_argument(
        ...
        default=_parse_image_size_arg("32"),
        help="Square size (e.g. 32) or HEIGHTxWIDTH (e.g. 32x32) for resizing input images.",
    )
